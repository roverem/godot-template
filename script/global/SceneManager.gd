...
func _add_loading_screen(_loading_container:Node, transition_type:String="fade_to_black"):
	...
	_loading_container.add_child(_loading_screen)
...
func swap_scenes(
	...
	var _add_loading_to:Node
	_loading_in_progress = true
	
	if load_into == null: 
		load_into = get_tree().root
		_add_loading_to = load_into
	else:
		_add_loading_to = load_into.get_parent()
		
	_load_scene_into = load_into
	_scene_to_unload = scene_to_unload
	
	_add_loading_screen(_add_loading_to, transition_type)
